...
(function(){Matter.JessPlugin={fromVerticesWithFallback:function(h,z,A,v,B,C,D,E,F,G,I){var J=function(e,f){if(null==e.parts||1>e.parts.length||2>e.parts.length)return!0;if(null!=f)for(var c=H(f),d=0;d<e.parts.length;d++){for(var l=e.parts[d],t=!1,g=0;g<c.length;g++)if(!0==s(l.position,c[g])){t=!0;break}if(!1==t)return!1}if(2>e.parts.length)return!0;c=[];for(d=0;d<e.parts.length;d++)l=e.parts[d],l==e||3>l.vertices.length||c.push(H(l));a:{for(d=0;d<c.length;d++)for(l=0;l<c.length;l++)if(t=c[d],g=c[l],t!=g)for(var w=
